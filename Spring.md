...
@NamedStoredProcedureQueries(value = {
    @NamedStoredProcedureQuery(name = "findPermissions", procedureName = "find_permissions",
        resultClasses = {Permission.class}),
    @NamedStoredProcedureQuery(name = "findPermissionIds", procedureName = "find_permission_ids")})
@NamedQuery(name = "Permission.findAll", query = "SELECT p FROM Permission p")
public class Permission implements Serializable {

}
```
* Extend the funcionality of the repository. Create an interface and an implementation class. The name of the implementation must be the same as the repository plus "Impl" (Spring will inject this implementation to the class automatically generated by Spring itself).
```java
...
public class PermissionRepositoryImpl implements PermissionRepositoryCustom {
  ...
  @PersistenceContext(unitName = "authorization")
  ...
  private <T> List<T> callProcedure(String userId, Class<T> clazz, String storedProcedureName) {
    ...
    if (execute) {
      ...
      log.error("Execution of stored procedure " + userId + " has failed");
    }
    return results;
  }
...
```
* The repository must extend our custom interface:
```java
@Repository
