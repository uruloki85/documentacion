...
1. [Essential commands](#essential-commands)
	...
	1. [About Postgres server](#about-server)
		1. [Check status](#server-status)
...
- `--disable-triggers`: disable triggers while loading the dump (only valid for a `--data-only` restore).
	- Requires using option `-S/--superuser` with the superuser to be used when disabling triggers.
- `-c/--clean`: clean (drop) database objects before recreating them (only valid for a `--schema-only` restore).
